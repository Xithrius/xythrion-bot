...
from lxml import etree  # pyright: ignore[reportAttributeAccessIssue, reportUnknownVariableType]
...
class LinkMapRedirect(Cog):
    """Replacing the contents of one link with another one."""
    ...
    def get_link_map_output_channel(self, discord_channel_id: int) -> int | None:
        if ((link_map_channels := self.link_map_channels) is not None) and (
            link_map_channel := link_map_channels.get(discord_channel_id)
        ):
    ...
    async def extract_url_from_webpage(self, url: str, xpath: str) -> str:
        ...
        dom = etree.HTML(str(soup), None)  # pyright: ignore[reportUnknownVariableType]
        extracted = dom.xpath(xpath)  # pyright: ignore[reportUnknownVariableType]

        # "src" for images, "data-src" for videos
        extracted_url = extracted[0].get("src") or extracted[0].get("data-src")  # pyright: ignore[reportUnknownVariableType]

        return extracted_url  # pyright: ignore[reportUnknownVariableType]

    async def cog_check(self, ctx: Context) -> bool:  # type: ignore[context]
        """Not allowing link map commands to be ran in DMs (yet)."""
    ...
    async def on_message(self, message: Message) -> None:
        ...
        for converter in converters:
            if converter.from_link in message.content:
                ...
                # This branch should also never be hit
                if new_url is None:
                    log.error(
                        f"Link converter with ID {converter.id} does not have a destination "
                        "attribute of `to_link` or `xpath`.",
                    )
                    return
    ...
    async def list_link_map_channels(self, ctx: Context, server_id: int | None = None) -> None:
        if server_id is None and (guild := ctx.guild) is not None:
            server_id = guild.id
        elif guild is None:  # pyright: ignore[reportPossiblyUnboundVariable]
            await ctx.error_embed("Link maps are not supported in DMs")
            return
...
async def setup(bot: Xythrion) -> None:
    await bot.add_cog(LinkMapRedirect(bot))
